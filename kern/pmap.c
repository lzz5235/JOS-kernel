...
mem_init(void)
{
    ...
    boot_map_region(kern_pgdir,
                    ...
                    PADDR(bootstack),
                    PTE_W);
	cprintf("bootstack:0x%x\n",bootstack);//变量声明的地址是在KERNBASE之后,但是求出物理地址后是在内存的开始处
    cprintf("PADDR(bootstack):0x%x\n",PADDR(bootstack));
//Virtual memory 布局可以随意安排，但是物理内存是紧紧挨着内存最开始地址的！
    //////////////////////////////////////////////////////////////////////
